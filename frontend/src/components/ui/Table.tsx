...
export const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        "h-12 px-3 md:px-6 align-middle font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider text-xs whitespace-nowrap",
        className
      )}
      {...props}
    />
...
export const TableCell = React.forwardRef<HTMLTableCellElement, React.TdHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <td ref={ref} className={cn("px-3 md:px-6 py-3 md:py-4 align-middle text-[var(--color-text-primary)]", className)} {...props} />
  )
);
